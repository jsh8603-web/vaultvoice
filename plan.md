...
| # | 테스트 | 검증 기준 | 커버 기능 |
|---|--------|----------|----------|
| T1 | 피드 탭 진입 | AI 요약/태그추천/분류 버튼 **미표시**, `#ai-result` 영역 없음 | 1-1 버튼 제거 |
| T2 | 카드 요약 버튼 (✦) 클릭 | DUMMY 카드에 `.card-summary` 인라인 표시, Gemini 응답 포함 | 1-2 요약 + 1-3 /api/note/summarize |
| T3 | 카드 삭제 버튼 (🗑) 클릭 | confirm 다이얼로그 → 확인 → 카드 사라짐 + 파일 삭제됨 | 1-2 삭제 + 1-3 /api/note/delete |
| T4 | 카드 삭제 — 취소 | confirm에서 취소 → 카드 유지, 파일 존재 | 1-2 삭제 안전장치 |
| T5 | 카드 Jarvis 버튼 (💬) 클릭 | AI 탭으로 전환 + 해당 노트 컨텍스트가 Jarvis 입력에 반영 | 1-2 Jarvis 연동 |
| T6 | 카드 코멘트 버튼 클릭 | 입력창 표시 → 자연어 입력 → 노트 파일에 `## 코멘트` 섹션 append | 코멘트 기능 |
| T7 | 삭제 보안: 99_vaultvoice 외 파일 | `/api/note/delete`에 다른 경로 파일명 전달 → 거부 | 보안 |

### 축2 테스트: Jarvis 도구

| # | 테스트 | 검증 기준 | 커버 도구 |
|---|--------|----------|----------|
| T8 | Jarvis: "TEST_memo 삭제해줘" | delete_note 호출 → 파일 삭제 확인 | delete_note |
| T9 | Jarvis: "오늘 할일 중 첫번째 완료 처리" | toggle_todo 호출 → 체크박스 토글 | toggle_todo |
| T10 | Jarvis: "오늘 할일 중 두번째 삭제" | delete_todo 호출 → 해당 라인 제거 | delete_todo |
| T11 | Jarvis: "TEST_voice 요약해줘" | summarize_note 호출 → AI 요약 응답 | summarize_note |
| T12 | Jarvis: URL 전달 | process_url 호출 → 새 노트 생성 + 요약 포함 | process_url |
| T13 | Jarvis: "이 노트에 '좋은 내용이다'라고 코멘트" | add_comment 호출 → 정리된 코멘트 append | add_comment |

### 축3 테스트: 신규 기능

| # | 테스트 | 검증 기준 | 커버 기능 |
|---|--------|----------|----------|
| T14 | Jarvis 온보딩 버튼 표시 | AI 탭 진입 시 기능 안내 버튼 5개 표시, 대화 시작 후 숨김 | G. 온보딩 |
| T15 | 온보딩 버튼 클릭 | 버튼 클릭 → Jarvis가 예시+설명 응답 | G. 온보딩 |
| T16 | 관련 노트 추천 | DUMMY 카드 상세 시 하단에 관련 노트 최대 3개 표시 | A. 관련 노트 |
| T17 | 검색 필터: 유형 | 검색어 입력 + filterType=voice → voice 노트만 결과 | E. 검색 필터 |
| T18 | 검색 필터: 기간 | 검색어 입력 + filterDate=7 → 최근 7일 노트만 결과 | E. 검색 필터 |
| T19 | 음성 전사 → 일정 감지 | "다음 수요일 3시 회의" 포함 전사 저장 → 일정 감지 팝업 | D. 일정 감지 |
| T20 | 노트 제목 자동생성 | 새 노트 저장 시 frontmatter에 `title` 필드 자동 추가 | F-1. 제목 생성 |
| T21 | 기존 노트 backfill | `/api/notes/backfill-titles` 호출 → DUMMY 노트에 title 추가됨 | F-1. backfill |
| T22 | 위키링크 삽입 | title이 있는 노트 본문에 다른 노트 title 언급 시 `[[링크]]` 변환 | F-2. 위키링크 |

### 인프라 테스트: Gemini 모델 티어링

| # | 테스트 | 검증 기준 | 커버 기능 |
|---|--------|----------|----------|
| T23 | Flash 호출 확인 | 코멘트 정리/태그 추출 시 서버 로그에 `gemini-2.0-flash` 표시 | 티어링 |
| T24 | Pro 호출 확인 | Jarvis 대화/URL 요약 시 서버 로그에 `gemini-2.5-pro` 표시 | 티어링 |
| T25 | Pro fallback | Pro 모델 장애 시뮬레이션 → Flash로 자동 재시도 | 티어링 fallback |

### 설정탭 연동 테스트

| # | 테스트 | 검증 기준 | 커버 기능 |
|---|--------|----------|----------|
| T26 | 기능 안내: 피드 탭 설명 | "카드별 AI 요약·삭제·코멘트" 문구 포함, 구 "AI 요약·태그 추천·분류" 미포함 | 안내 텍스트 |
| T27 | 기능 안내: Jarvis 설명 | "노트 요약·삭제·코멘트, URL 처리" 등 신규 기능 반영 | 안내 텍스트 |
| T28 | 전체 기능 점검 버튼 | Gemini Flash + Pro 연결 OK, 신규 엔드포인트 OK 표시 | 기능 테스트 확장 |
| T29 | 코멘트 보안: path traversal | `/api/note/comment`에 `../` 포함 파일명 → 거부 | 보안 |

### 크로스 기능 E2E 테스트 (기능 간 연계)

| # | 테스트 | 검증 기준 | 커버 기능 |
|---|--------|----------|----------|
| T30 | 피드 카드 Jarvis → 코멘트 | 카드에서 💬 Jarvis → AI탭 전환 → "이 노트에 코멘트해줘" → add_comment 작동 | 축1↔축2 연계 |
| T31 | process_url → 제목 자동생성 | Jarvis로 URL 처리 → 새 노트에 title 자동 포함 | process_url + F-1 |
| T32 | 제목 backfill → 관련 노트 추천 | backfill로 title 추가된 DUMMY → 관련 노트에 title 표시 | F-1 + A |
| T33 | 코멘트 2회 연속 추가 | 같은 노트에 코멘트 2번 → `## 코멘트` 섹션에 2줄 모두 타임스탬프 포함 | 코멘트 중복 |
| T34 | 온보딩 → 실제 기능 실행 | 온보딩 "URL 저장·요약" 클릭 → 설명 확인 → 실제 URL 전달 → 정상 처리 | G → process_url |
| T35 | 검색 필터 + Jarvis search 정합성 | 동일 키워드로 검색탭 필터 결과 vs Jarvis search 결과 비교 | E + Jarvis search |
